...
import { commentOnBlogPost } from '@/actions/blogCommentAction';
function BlogCommentForm({post_id}) {
    const initialState = {
        name: '',
        email: '',
        comment: '',
    ...
    const [state, action, isPending] = useActionState(commentOnBlogPost, initialState)
       ...
       <h2 className="text-lg font-semibold text-gray-800">Leave a Comment</h2>
        ...
        <form action={action} className='space-y-4 flex flex-col gap-2 '>
            ...
            <div className='hidden'>
               <input type="hidden" id="post_id" name="post_id" defaultValue={post_id} />
            </div>
