...
export async function commentOnJob(state, formData) {
  console.log(formData.get("email"), formData.get("name"));

  let job_id = formData.get("job_id")
...
if (!job_id){
  ...
  if (Object.keys(errors).length > 0) {
   ...
   let result = await DatabaseFunctions.InsertComment(job_id,commetor_name,commetor_email, commetor_comment)
  if (result.success){
    return{
        sucess:true,
        error:false
